...
public interface EntityHierarchyInfoRepository extends JpaRepository<EntityHierarchyInfo, Long> {
        ...
        @Query(value = """
                        ...
                        """, nativeQuery = true)
        EntityHierarchyInfo findByHierarchyId(
        ...
        @Query(value = """
                        SELECT *
                        FROM entity_hierarchy_info
                        WHERE ehi_companyid = :companyId
                        ORDER BY ehi_level_no DESC
                        LIMIT 1
                        """, nativeQuery = true)
        Optional<EntityHierarchyInfo> findLatestHierarchyByCompanyId(
